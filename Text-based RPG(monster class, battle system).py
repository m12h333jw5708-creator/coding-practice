...
def battle(player, monster_list):
    a = random.choice(monster_list)
    print(f"{a.name} appeared!")
    print("Battle Start!")
    print(f"Monster HP: {a.hp}")
    while True:
        try:
            print("====== Battle Menu =====")
            print("1. attack")
            ...
            if user == 1:
                if (player.atk - a.def_stat) <= 0:
                    print("Monster's defense is too strong to attack")
                elif (player.atk - a.def_stat) > 0:
                    a.hp -= (player.atk - a.def_stat)
                    if a.hp <= 0:
                        print("Victory!!")
                        player.exp += a.exp
                        print(f"EXP +{a.exp}")
                        ...
                        print(f"Monster's remaining physical strength : {a.hp}")
                        if (a.atk - player.def_stat) > 0:
                            player.hp -= (a.atk - player.def_stat)
                        elif (a.atk - player.def_stat) <= 0:
